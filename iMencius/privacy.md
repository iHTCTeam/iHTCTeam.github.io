---
layout: doc
title: iMencius — Privacy Policy
app_id: iMencius
doc_title_en: Privacy Policy
doc_title_zh_cn: 隐私政策
doc_title_zh_tw: 隱私政策
description: Privacy policy for iMencius (爱孟子) — fully offline, Data Not Collected, optional iCloud and Photo Library add-only.
---
...
| Network | Library, reading, recite, Nurturing Qi Study, widgets, and daily notifications work **offline** after install |
| Analytics / ads / tracking | **None** — no third-party analytics, advertising, or tracking SDKs; no App Tracking Transparency use |
| Optional Apple services | **iCloud** (your private KVS), **Photo Library (Add Only)** for 孟 cards, **StoreKit** for optional tipping, **local notifications**, optional **Live Activities**, optional **on-device handwriting** (Vision) |
...
- **Widgets & App Group** — Widgets and the app share preferences and lightweight state via App Group `group.com.iHTCboy.iMencius` on your device.
...
- **孟 cards** — Images are rendered on device. Saving uses **Photo Library add-only** only when you tap save. System Share Sheet may send the image to an app **you** choose; that destination’s privacy policy then applies.
...
| Data | Where | Purpose |
|------|-------|---------|
| Mencius database | App / Widget bundle | Offline reading & study |
| Favorites & lists (读孟单 lists) | App Group UserDefaults | Starred passages and user lists |
...
| Daily Saying cache & notification prefs | App Group UserDefaults | Today’s ID, schedule time, contextual toggle |
...
iMencius requests permissions only when a feature needs them:

| Permission | When | Why |
|------------|------|-----|
| Notifications | You enable Daily Saying reminders | Local scheduling of reminder notifications |
...
iMencius is an educational literacy app for a general audience and is **not** in the Kids Category. We do not knowingly collect personal information from children. The app does not require an account. Families may use reading and study features offline on their own devices.
...
爱火腿肠工作室（iHTC Studio）（「我们」；原 iHTCTeam）开发了 **爱孟子**（iMencius / 愛孟子，Bundle ID `com.iHTCboy.iMencius`）。本隐私政策说明你在使用本应用时，数据如何被处理。本页可作为 **App Store 审核**与用户查阅的公开隐私政策地址。在 App Store Connect 填写时：打开本站[隐私政策](privacy)页，从浏览器地址栏复制当前网址（站点域名日后可能变更，请勿依赖写死的完整域名）。
...
| 网络 | 语料、品读、诵读、养气学堂、小组件与每日通知安装后均可**离线** |
| 分析 / 广告 / 追踪 | **无** — 无第三方分析、广告或追踪 SDK；不使用 ATT |
| 可选 Apple 服务 | **iCloud**（你的私人 KVS）、保存 孟卡 时的**照片图库（仅添加）**、可选 **StoreKit** 打赏、**本地通知**、可选**实时活动**、可选本机**手写识别**（Vision） |
...
- **小组件与 App Group** — 通过本机 App Group `group.com.iHTCboy.iMencius` 共享偏好与轻量状态。
- **每日通知** — 使用本地 User Notifications 调度。点按通知通过深链打开本机篇目 ID；无服务器推送。
- **孟卡** — 在设备上渲染。仅在你点保存时使用**照片图库（仅添加）**。系统分享表可能把图片发给**你选择**的 App，随后适用对方隐私政策。
...
| 数据 | 位置 | 用途 |
|------|------|------|
| 《孟子》语料 | App／小组件 Bundle | 离线阅读与学习 |
| 收藏与清单（读孟单） | App Group UserDefaults | 星标篇目与用户清单 |
...
| 每日一语 缓存与通知偏好 | App Group UserDefaults | 今日 ID、提醒时间、情境开关 |
...
爱孟子 是面向大众的教育类读写应用，**不属于** Kids Category。我们不会有意收集儿童的个人信息。应用不要求账号。家庭可在自有设备上离线使用阅读与学习功能。
...
愛火腿腸工作室（iHTC Studio）（「我們」；原 iHTCTeam）開發了 **愛孟子**（iMencius / 爱孟子，Bundle ID `com.iHTCboy.iMencius`）。本隱私政策說明你在使用本應用時，資料如何被處理。本頁可作為 **App Store 審核**與使用者查閱的公開隱私政策地址。在 App Store Connect 填寫時：打開本站[隱私政策](privacy)頁，從瀏覽器網址列複製目前網址（站點網域日後可能變更，請勿依賴寫死的完整網域）。
...
| 網路 | 語料、品讀、誦讀、養氣學堂、小工具與每日通知安裝後均可**離線** |
| 分析 / 廣告 / 追蹤 | **無** — 無第三方分析、廣告或追蹤 SDK；不使用 ATT |
| 可選 Apple 服務 | **iCloud**（你的私人 KVS）、儲存 孟卡 時的**照片圖庫（僅加入）**、可選 **StoreKit** 打賞、**本機通知**、可選**即時動態**、可選本機**手寫辨識**（Vision） |
...
- **小工具與 App Group** — 透過本機 App Group `group.com.iHTCboy.iMencius` 共享偏好與輕量狀態。
- **每日通知** — 使用本機 User Notifications 排程。點按通知透過深鏈打開本機篇目 ID；無伺服器推播。
- **孟卡** — 在裝置上渲染。僅在你點儲存時使用**照片圖庫（僅加入）**。系統分享表可能把圖片傳給**你選擇**的 App，隨後適用對方隱私政策。
...
| 資料 | 位置 | 用途 |
|------|------|------|
| 《孟子》語料 | App／小工具 Bundle | 離線閱讀與學習 |
| 收藏與清單（讀孟單） | App Group UserDefaults | 星標篇目與使用者清單 |
...
| 每日一語 快取與通知偏好 | App Group UserDefaults | 今日 ID、提醒時間、情境開關 |
...
愛孟子 是面向大眾的教育類讀寫應用，**不屬於** Kids Category。我們不會有意蒐集兒童的個人資訊。應用不要求帳號。家庭可在自有裝置上離線使用閱讀與學習功能。
