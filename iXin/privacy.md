---
layout: doc
title: iXin — Privacy Policy
app_id: iXin
doc_title_en: Privacy Policy
doc_title_zh_cn: 隐私政策
doc_title_zh_tw: 隱私政策
description: Privacy policy for iXin (爱王阳明) — fully offline, Data Not Collected, optional iCloud and Photo Library add-only.
---
...
| Network | Library, reading, recite, Heart School, journal, widgets, and daily notifications work **offline** after install |
| Analytics / ads / tracking | **None** — no third-party analytics, advertising, or tracking SDKs; no App Tracking Transparency use |
| Optional Apple services | **iCloud** (your private KVS), **Photo Library (Add Only)** for insight cards, **StoreKit** for optional tipping, **local notifications**, optional **Live Activities**, optional **on-device handwriting** (Vision) |
...
- **Widgets & App Group** — Widgets and the app share preferences and lightweight state via App Group `group.com.iHTCboy.iXin` on your device.
...
- **Insight cards** — Images are rendered on device. Saving uses **Photo Library add-only** only when you tap save. System Share Sheet may send the image to an app **you** choose; that destination’s privacy policy then applies.
...
- **Knowing-Acting journal** — Entries you write stay on device (App Group). If iCloud Sync is on, they are mirrored through **your** private iCloud KVS, not to iHTC Studio.

## Data Stored on Your Device

| Data | Where | Purpose |
|------|-------|---------|
| Yangming database | App / Widget bundle | Offline reading & study |
| Favorites & lists (心单) | App Group UserDefaults | Starred passages and user lists |
...
| Study progress & mastery | App Group UserDefaults | Heart School state, quiz / Hunt progress |
...
| Daily Insight cache & notification prefs | App Group UserDefaults | Today’s ID, schedule time, contextual toggle |
...
| Knowing-Acting journal | App Group UserDefaults | Daily known / acted / reflect entries |
...
- Practice streaks
- Notes
- Knowing-Acting journal entries
...
iXin requests permissions only when a feature needs them:

| Permission | When | Why |
|------------|------|-----|
| Notifications | You enable Daily Insight reminders | Local scheduling of reminder notifications |
| Photo Library (Add Only) | You save a card | Write the rendered image; we do not need full library read access |
| iCloud | You enable sync | Sync favorites / lists / history / progress / streaks / notes / journal |
...
iXin is an educational literacy app for a general audience and is **not** in the Kids Category. We do not knowingly collect personal information from children. The app does not require an account. Families may use reading and study features offline on their own devices.
...
爱火腿肠工作室（iHTC Studio）（「我们」；原 iHTCTeam）开发了 **爱王阳明**（iXin / 愛王陽明，Bundle ID `com.iHTCboy.iXin`）。本隐私政策说明你在使用本应用时，数据如何被处理（以及**不会**如何被处理）。本页可作为 **App Store 审核**与用户查阅的公开隐私政策。填写 App Store Connect 时，请在浏览器打开本 [隐私政策](privacy) 页，从地址栏复制当前网址（站点域名日后可能变更，请勿依赖文档中的固定域名）。
...
| 网络 | 语料、阅读、诵读、心学堂、知行日记、小组件与每日通知在安装后可**离线**使用 |
| 分析 / 广告 / 追踪 | **无**——无第三方分析、广告或追踪 SDK；不使用跨 App 追踪 |
| 可选 Apple 服务 | **iCloud**（你的私有键值存储）、保存心卡时的**相簿（仅加入）**、可选 **StoreKit** 打赏、**本地通知**、可选**实时活动**、可选本机**手写识别**（Vision） |
...
- **不上传内容** — 阅读历史、收藏、心单、笔记、学堂进度与偏好留在本机（以及可选的**你的** iCloud）。它们不会上传到 iHTC Studio 服务器，因为**我们不为该 App 运营此类服务器**。

## 应用如何工作

- **内置语料** — 正文 JSON、白话 / 英译、索引与学习辅助随 App / 小组件包分发。浏览与学堂**无需**网络请求。
- **本机语音** — 诵读使用系统 **AVSpeechSynthesizer**（TTS）。我们不会把正文发到 iHTC Studio。
- **小组件与 App Group** — 小组件与主应用通过本机 App Group `group.com.iHTCboy.iXin` 共享偏好与轻量状态。
- **每日通知** — 使用 User Notifications **本机排程**。点按通知会打开指向本机篇目 ID 的深链，没有服务端推送名单。
- **心卡** — 图片在本机渲染。仅在你点保存时使用**相簿仅加入**。系统分享表可能把图片发到**你选择**的 App，随后适用该目标的隐私政策。
...
- **知行日记** — 你写下的条目留在本机（App Group）。若开启 iCloud 同步，会镜像到**你的**私有 iCloud 键值存储，而不是 iHTC Studio。

## 本机存储的数据

| 数据 | 位置 | 用途 |
|------|------|------|
| 心学语料 | App / Widget Bundle | 离线阅读与学堂 |
| 收藏与心单 | App Group UserDefaults | 星标与自订列表 |
| 浏览纪录 / 最近阅读 | App Group UserDefaults | 继续阅读与历史 |
| 笔记 | App Group UserDefaults | 你写下的阅读心得 |
| 学堂进度与掌握度 | App Group UserDefaults | 心学堂状态、测验 / 寻心进度 |
| 打卡 | App Group UserDefaults | 每日习惯 |
| 外观与阅读偏好 | App Group UserDefaults | 主题、强调色、语言、版面、朗读、Tab、小组件、字级 |
| 每日一心缓存与通知偏好 | App Group UserDefaults | 今日 ID、提醒时间、情境开关 |
| 心卡偏好 | App Group UserDefaults | 上次风格 / 比例 |
| 引导与 TipKit 状态 | 标准 UserDefaults | 欢迎引导 / 发现提示 |
| 最近搜索 | 标准 UserDefaults | 你输入过的搜索词 |
| 主屏快捷 | 系统 + 本机偏好 | 你设定的快捷 |
| 小组件 / 实时活动快照 | App Group / ActivityKit | 展示你已打开的正文 |
| 知行日记 | App Group UserDefaults | 知 / 行 / 省 日记条目 |

解除安装会清除与该 App 相关的沙盒与 App Group 数据（以系统行为为准）。可选 iCloud 副本仍留在**你的** iCloud。

## iCloud 同步（可选）

若你在设置中开启 **iCloud 同步**，应用会通过绑定**你的 Apple ID** 的 Apple **`NSUbiquitousKeyValueStore`** 镜像：收藏、心单、阅读历史、学堂进度／掌握度、打卡与笔记，以及**知行日记**。同步流量走向 **Apple iCloud**，而不是 iHTC Studio。我们无法读取你的 iCloud 内容。
...
|------|------|------|
| 通知 | 你开启每日一心提醒 | 本地排程提醒 |
| 相簿（仅加入） | 你保存心卡 | 写入渲染图片；无需读取全部照片 |
| iCloud | 你开启同步 | 同步收藏／心单／历史／进度／打卡／笔记／知行日记 |
...
- **不**在你主动保存心卡之外访问照片（且仅为「加入」）。
- **不**接收 StoreKit 银行卡号码（由 Apple 处理付款）。
- **不**把手写笔迹或正文上传到 iHTC Studio。

## 儿童隐私

爱王阳明是面向一般受众的语文学习应用，**不属于儿童类别**。我们不会故意收集包括 13 岁以下（或你所在地区同等年龄）儿童在内的任何人的个人信息。应用不需要账号。家庭可在自有设备上离线使用阅读与学堂功能。
...
愛火腿腸工作室（iHTC Studio）（「我們」；原 iHTCTeam）開發了 **愛王陽明**（iXin / 爱王阳明，Bundle ID `com.iHTCboy.iXin`）。本隱私政策說明你在使用本應用時，資料如何被處理（以及**不會**如何被處理）。本頁可作為 **App Store 審核**與使用者查閱的公開隱私政策。填寫 App Store Connect 時，請在瀏覽器打開本 [隱私政策](privacy) 頁，從網址列複製目前網址（站點網域日後可能變更，請勿依賴文件中的固定網域）。
...
| 網路 | 語料、閱讀、誦讀、心學堂、知行日記、小工具與每日通知在安裝後可**離線**使用 |
| 分析 / 廣告 / 追蹤 | **無**——無第三方分析、廣告或追蹤 SDK；不使用跨 App 追蹤 |
| 可選 Apple 服務 | **iCloud**（你的私有鍵值儲存）、儲存心卡時的**相簿（僅加入）**、可選 **StoreKit** 打賞、**本地通知**、可選**即時動態**、可選本機**手寫辨識**（Vision） |
...
- **不上傳內容** — 閱讀歷史、收藏、心單、筆記、學堂進度與偏好留在本機（以及可選的**你的** iCloud）。它們不會上傳到 iHTC Studio 伺服器，因為**我們不為該 App 營運此類伺服器**。

## 應用如何工作

- **內建語料** — 正文 JSON、白話／英譯、索引與學習輔助隨 App／小工具包分發。瀏覽與學堂**無需**網路請求。
- **本機語音** — 誦讀使用系統 **AVSpeechSynthesizer**（TTS）。我們不會把正文發到 iHTC Studio。
- **小工具與 App Group** — 小工具與主應用透過本機 App Group `group.com.iHTCboy.iXin` 共享偏好與輕量狀態。
- **每日通知** — 使用 User Notifications **本機排程**。點按通知會打開指向本機篇目 ID 的深鏈，沒有伺服器推送名單。
- **心卡** — 圖片在本機渲染。僅在你點儲存時使用**相簿僅加入**。系統分享表可能把圖片發到**你選擇**的 App，隨後適用該目標的隱私政策。
...
- **知行日記** — 你寫下的條目留在本機（App Group）。若開啟 iCloud 同步，會鏡像到**你的**私有 iCloud 鍵值儲存，而不是 iHTC Studio。

## 本機儲存的資料

| 資料 | 位置 | 用途 |
|------|------|------|
| 心學語料 | App / Widget Bundle | 離線閱讀與學堂 |
| 收藏與心單 | App Group UserDefaults | 星標與自訂列表 |
| 瀏覽紀錄 / 最近閱讀 | App Group UserDefaults | 繼續閱讀與歷史 |
| 筆記 | App Group UserDefaults | 你寫下的閱讀心得 |
| 學堂進度與掌握度 | App Group UserDefaults | 心學堂狀態、測驗 / 尋心進度 |
| 打卡 | App Group UserDefaults | 每日習慣 |
| 外觀與閱讀偏好 | App Group UserDefaults | 主題、強調色、語言、版面、朗讀、Tab、小工具、字級 |
| 每日一心快取與通知偏好 | App Group UserDefaults | 今日 ID、提醒時間、情境開關 |
| 心卡偏好 | App Group UserDefaults | 上次風格 / 比例 |
...
| 知行日記 | App Group UserDefaults | 知 / 行 / 省 日記條目 |

解除安裝會清除與該 App 相關的沙盒與 App Group 資料（以系統行為為準）。可選 iCloud 副本仍留在**你的** iCloud。

## iCloud 同步（可選）

若你在設定中開啟 **iCloud 同步**，應用會透過綁定**你的 Apple ID** 的 Apple **`NSUbiquitousKeyValueStore`** 鏡像：收藏、心單、閱讀歷史、學堂進度／掌握度、打卡與筆記，以及**知行日記**。同步流量走向 **Apple iCloud**，而不是 iHTC Studio。我們無法讀取你的 iCloud 內容。
...
|------|------|------|
| 通知 | 你開啟每日一心提醒 | 本地排程提醒 |
| 相簿（僅加入） | 你儲存心卡 | 寫入渲染圖片；無需讀取全部照片 |
| iCloud | 你開啟同步 | 同步收藏／心單／歷史／進度／打卡／筆記／知行日記 |
...
- **不**在你主動儲存心卡之外存取照片（且僅為「加入」）。
- **不**接收 StoreKit 銀行卡號碼（由 Apple 處理付款）。
- **不**把手寫筆跡或正文上傳到 iHTC Studio。

## 兒童隱私

愛王陽明是面向一般受眾的語文學習應用，**不屬於兒童類別**。我們不會故意收集包括 13 歲以下（或你所在地區同等年齡）兒童在內的任何人的個人資訊。應用不需要帳號。家庭可在自有裝置上離線使用閱讀與學堂功能。
