---
layout: doc
title: iSage — Privacy Policy
app_id: iSage
doc_title_en: Privacy Policy
doc_title_zh_cn: 隐私政策
doc_title_zh_tw: 隱私政策
description: Privacy policy for iSage (爱增广贤文) — fully offline Zengguang Xianwen app, Data Not Collected, optional iCloud and Photo Library add-only.
---
...
| Network | Library, reading, recite, Sage Academy, widgets, and daily notifications work **offline** after install |
| Analytics / ads / tracking | **None** — no third-party analytics, advertising, or tracking SDKs; no App Tracking Transparency use |
| Optional Apple services | **iCloud** (your private KVS), **Photo Library (Add Only)** for sage cards, **StoreKit** for optional tipping, **local notifications**, optional **Live Activities**, optional **on-device handwriting** (Vision) |
...
- **Bundled library** — Saying JSON, vernacular/English text, sage readings, life scenes, and indexes ship inside the app / widget bundle. Browsing and study do not require a network request.
- **On-device speech** — Recitation uses Apple’s system **AVSpeechSynthesizer** (TTS). We do not send saying text to iHTC Studio.
- **Widgets & App Group** — Widgets and the app share preferences and lightweight state via App Group `group.com.iHTCboy.iSage` on your device.
- **Daily notifications** — Scheduled locally with User Notifications. Tapping a notification opens a deep link to a saying ID on device; no server fan-out.
- **Sage cards** — Images are rendered on device. Saving uses **Photo Library add-only** only when you tap save. System Share Sheet may send the image to an app **you** choose; that destination’s privacy policy then applies.
...
| Data | Where | Purpose |
|------|-------|---------|
| Saying database | App / Widget bundle | Offline reading & study |
| Favorites & lists (贤单) | App Group UserDefaults | Starred sayings and user lists |
...
| Study progress & mastery | App Group UserDefaults | Sage Academy state, quiz / Hunt progress |
...
| Today’s Wisdom cache & notification prefs | App Group UserDefaults | Today’s ID, schedule time, contextual toggle |
| Sage-card preferences | App Group UserDefaults | Last style / ratio choices |
...
iSage requests permissions only when a feature needs them:

| Permission | When | Why |
|------------|------|-----|
| Notifications | You enable Today’s Wisdom reminders | Local scheduling of reminder notifications |
| Photo Library (Add Only) | You save a sage card | Write the rendered image; we do not need full library read access |
| iCloud | You enable sync | Sync favorites / lists / history / progress / streaks / notes |
| Live Activities (iOS) | You enable Reading Live Activity | Show the saying you are reading on Lock Screen / Dynamic Island |
...
- We do **not** upload handwriting strokes or saying text to iHTC Studio.

## Children’s Privacy

iSage is an educational literacy app for a general audience and is **not** in the Kids Category. We do not knowingly collect personal information from children. The app does not require an account. Families may use reading and study features offline on their own devices.
...
爱火腿肠工作室（iHTC Studio）（「我们」；原 iHTCTeam）开发了 **爱增广贤文**（iSage / 愛增廣賢文，Bundle ID `com.iHTCboy.iSage`）。本隐私政策说明你在使用本应用时，数据如何被处理（以及**不会**如何被处理）。本页可作为 **App Store 审核**与用户查阅的公开隐私政策。填写 App Store Connect 时，请在浏览器打开本 [隐私政策](privacy) 页，从地址栏复制当前网址（站点域名日后可能变更，请勿依赖文档中的固定域名）。
...
| 网络 | 语料、阅读、诵读、贤文学堂、小组件与每日通知在安装后可**离线**使用 |
| 分析 / 广告 / 追踪 | **无**——无第三方分析、广告或追踪 SDK；不使用跨 App 追踪 |
| 可选的 Apple 服务 | **iCloud**（你的私有键值存储）、贤卡**相册（仅添加）**、打赏用的 **StoreKit**、**本地通知**、可选**实时活动**、可选本机**手写识别**（Vision） |
...
- **无内容上传** — 浏览记录、收藏、贤单、笔记、学堂进度与偏好保存在你的设备（以及可选的**你的** iCloud）。它们不会上传到 iHTC Studio 服务器——因为**本应用不为此运营此类服务器**。

## 应用工作原理

- **内置语料** — 贤句 JSON、白话/英译、贤者解读、生活场景与索引随 App / Widget 安装包提供，浏览与学堂无需联网。
- **本机语音合成** — 诵读使用系统 **AVSpeechSynthesizer**，我们不会把正文发送到 iHTC Studio。
- **小组件与 App Group** — App 与小组件通过本机 App Group `group.com.iHTCboy.iSage` 共享偏好与轻量状态。
- **每日通知** — 使用本地用户通知调度；点击后通过深链打开本机贤句，无服务器推送内容库。
- **贤卡** — 图片在设备上渲染。仅当你点「保存」时请求**相册仅添加**权限。系统分享会把图片交给**你选择**的目标 App，此后适用该目标的隐私政策。
- **手写寻贤（iOS）** — 可选手写识别使用 Apple **Vision**，在本机完成；笔迹不会上传。
...
| 数据 | 位置 | 用途 |
|------|------|------|
| 贤句数据库 | App / Widget Bundle | 离线阅读与学堂 |
| 收藏与贤单 | App Group UserDefaults | 星标与自定义贤单 |
| 浏览记录 / 最近阅读 | App Group UserDefaults | 继续阅读与历史 |
| 笔记 | App Group UserDefaults | 你写下的阅读心得 |
| 学堂进度与掌握度 | App Group UserDefaults | 学堂状态、测验 / 寻贤进度 |
| 践行打卡 | App Group UserDefaults | 每日习惯与连续天数 |
| 外观与阅读偏好 | App Group UserDefaults | 主题、强调色、语言、布局、朗读、Tab、小组件、字号 |
| 今日一智缓存与通知偏好 | App Group UserDefaults | 今日 ID、提醒时间、情境开关 |
| 贤卡偏好 | App Group UserDefaults | 上次风格 / 比例 |
...
卸载应用会清除与该 App 相关的沙盒与 App Group 数据（以系统行为为准）。可选 iCloud 副本仍留在**你的** iCloud，直到你关闭同步并在他处清除。

## iCloud 同步（可选）

若你在设置中开启 **iCloud 同步**，应用会通过绑定**你的 Apple ID** 的 Apple **`NSUbiquitousKeyValueStore`** 镜像下列数据：
...
同步流量走向 **Apple iCloud**，而不是 iHTC Studio。可随时在设置中关闭。我们无法读取你的 iCloud 内容。
...
|------|------|------|
| 通知 | 你开启今日一智提醒 | 本地排程提醒 |
| 相册（仅添加） | 你保存贤卡 | 写入渲染图片；无需读取全部照片 |
| iCloud | 你开启同步 | 同步收藏 / 贤单 / 历史 / 进度 / 打卡 / 笔记 |
| 实时活动（iOS） | 你开启品读实时活动 | 在锁屏 / 灵动岛显示正在品读的贤句 |
| 背景音频能力 | 你开启后台诵读 | App 不在前台时继续 TTS |

我们**不**请求通讯录、精确位置、相机、麦克风（除系统 TTS 播放外）或追踪权限。手写寻贤使用本机 Vision，无需联网权限弹窗。
...
- **不**在你主动保存贤卡之外访问照片（且仅为「添加」）。
- **不**接收 StoreKit 银行卡号码（由 Apple 处理付款）。
- **不**把手写笔迹或贤句正文上传到 iHTC Studio。

## 儿童隐私

爱增广贤文是面向一般受众的语文学习应用，**不属于儿童类别**。我们不会故意收集包括 13 岁以下（或你所在地区同等年龄）儿童在内的任何人的个人信息。应用不需要账号。家庭可在自有设备上离线使用阅读与学堂功能。
...
愛火腿腸工作室（iHTC Studio）（「我們」；原 iHTCTeam）開發了 **愛增廣賢文**（iSage，Bundle ID `com.iHTCboy.iSage`）。本隱私政策說明你在使用本應用時，資料如何被處理。本頁可作為 **App Store 審核**與使用者查閱的公開隱私政策。在 App Store Connect 填寫隱私政策 URL 時，請在瀏覽器中開啟本 [隱私政策](privacy) 頁，從網址列複製目前網址（公開站點網域日後可能變更）。

## 概要（對齊 App Store 隱私標籤）

| 主題 | 我們的做法 |
|------|------------|
| 帳號 | **不需要**——無需註冊、電子郵件登入或手機號 |
| iHTC Studio 收集資料 | **不收集資料（Data Not Collected）**——我們**不**營運接收你個人資料的後端 |
| 網路 | 語料、閱讀、誦讀、賢文學堂、小工具與每日通知在安裝後可**離線**使用 |
| 分析 / 廣告 / 追蹤 | **無**——無第三方分析、廣告或追蹤 SDK；不使用跨 App 追蹤 |
| 可選的 Apple 服務 | **iCloud**（你的私有鍵值儲存）、賢卡**相簿（僅加入）**、打賞用的 **StoreKit**、**本地通知**、可選**即時動態**、可選本機**手寫辨識**（Vision） |
...
- **無內容上傳** — 瀏覽紀錄、收藏、賢單、筆記、學堂進度與偏好保存在你的裝置（以及可選的**你的** iCloud）。它們不會上傳到 iHTC Studio 伺服器——因為**本應用不為此營運此類伺服器**。

## 應用工作原理

- **內建語料** — 賢句 JSON、白話／英譯、賢者解讀、生活場景與索引隨 App / Widget 安裝包提供。
- **本機語音合成** — 誦讀使用系統 **AVSpeechSynthesizer**，我們不會把正文傳送到 iHTC Studio。
- **小工具與 App Group** — 透過 `group.com.iHTCboy.iSage` 在本機共享狀態。
- **每日通知** — 僅本機排程；點按後以深鏈開啟本機賢句。
- **賢卡** — 圖片在裝置上渲染。僅當你點「儲存」時請求**相簿僅加入**。系統分享交給**你選擇**的目標 App。
- **手寫尋賢（iOS）** — 可選手寫辨識使用 Apple **Vision**，在本機完成；筆跡不會上傳。
- **可選打賞** — 由 Apple **StoreKit** 處理支付；我們不會收到你的銀行卡資訊。
- **Siri / 捷徑 / 控制中心** — 透過本機深鏈開啟功能。
- **可選 Spotlight** — 由系統在本機建立搜尋索引。

## 裝置本機儲存

| 資料 | 位置 | 用途 |
|------|------|------|
| 賢句資料庫 | App / Widget Bundle | 離線閱讀與學堂 |
| 收藏與賢單 | App Group UserDefaults | 星標與自訂賢單 |
| 瀏覽紀錄 / 最近閱讀 | App Group UserDefaults | 繼續閱讀與歷史 |
| 筆記 | App Group UserDefaults | 你寫下的閱讀心得 |
| 學堂進度與掌握度 | App Group UserDefaults | 學堂狀態、測驗 / 尋賢進度 |
| 打卡 | App Group UserDefaults | 每日習慣 |
| 外觀與閱讀偏好 | App Group UserDefaults | 主題、強調色、語言、版面、朗讀、Tab、小工具、字級 |
| 今日一智快取與通知偏好 | App Group UserDefaults | 今日 ID、提醒時間、情境開關 |
| 賢卡偏好 | App Group UserDefaults | 上次風格 / 比例 |
...
若你在設定中開啟 **iCloud 同步**，應用會透過綁定**你的 Apple ID** 的 Apple **`NSUbiquitousKeyValueStore`** 鏡像：收藏、賢單、閱讀歷史、學堂進度／掌握度、打卡與筆記。同步流量走向 **Apple iCloud**，而不是 iHTC Studio。我們無法讀取你的 iCloud 內容。
...
|------|------|------|
| 通知 | 你開啟今日一智提醒 | 本地排程提醒 |
| 相簿（僅加入） | 你儲存賢卡 | 寫入渲染圖片；無需讀取全部照片 |
| iCloud | 你開啟同步 | 同步收藏／賢單／歷史／進度／打卡／筆記 |
| 即時動態（iOS） | 你開啟品讀即時動態 | 在鎖定畫面／動態島顯示正在品讀的賢句 |
...
- **不**在你主動儲存賢卡之外存取照片（且僅為「加入」）。
- **不**接收 StoreKit 銀行卡號碼（由 Apple 處理付款）。
- **不**把手寫筆跡或賢句正文上傳到 iHTC Studio。

## 兒童隱私

愛增廣賢文是面向一般受眾的語文學習應用，**不屬於兒童類別**。我們不會故意收集包括 13 歲以下（或你所在地區同等年齡）兒童在內的任何人的個人資訊。應用不需要帳號。家庭可在自有裝置上離線使用閱讀與學堂功能。
