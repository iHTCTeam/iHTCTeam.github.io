...
Edit Projects is a full non-linear editor, separate from the single-clip tools. Open it from **Library → Edit Projects** (top-right).
...
剪辑工程是完整的非线性编辑器，独立于单视频工具，入口在 **视频库 → 剪辑工程**（右上角）。
...
剪輯工程是完整的非線性編輯器，獨立於單影片工具，入口在 **影片庫 → 剪輯工程**（右上角）。
