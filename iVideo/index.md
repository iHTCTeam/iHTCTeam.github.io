...
**At a glance:** optional Record & Live Photo tabs · 31 tools in 7 categories · dedicated Edit Projects library · multi-track timelines · on-device subtitles (60+ languages) · face blur · color grading + LUTs · batch processing · 6 export presets + background queue · JKL pro playback · Siri · optional iCloud sync.
...
| **Edit Projects** | Dedicated project library — create, rename, duplicate, and open multi-track timelines |
| **Record** *(optional, off by default)* | Start a screen recording; the tab badge shows live elapsed time. Enable in Settings. |
| **Live Photo** *(optional, on by default)* | Convert to/from Live Photos; hide the tab in Settings if unused |
| **Settings** | Theme, language, export, recording, storage, help, about, tab visibility |
...
**一览：** 可选录屏与实况标签 · 7 大类 31 种工具 · 独立剪辑工程库 · 多轨时间线 · 设备端字幕（60+ 语言）· 人脸打码 · 调色 + LUT · 批处理 · 6 档导出 + 后台队列 · JKL 专业播放 · Siri · 可选 iCloud 同步。
...
| **剪辑工程** | 独立工程库——新建、重命名、复制，并打开多轨时间线 |
| **录屏**（可选，默认关闭） | 开始屏幕录制；Tab 角标实时显示已录时长；可在设置中开启 |
| **Live Photo**（可选，默认开启） | Live Photo 双向转换；不用时可在设置中隐藏 |
| **设置** | 主题、语言、导出、录屏、存储、帮助、关于、标签显隐 |
...
需要真正的时间线剪辑时，打开 **剪辑工程** 标签（或从视频库进入）。工程库支持重命名与复制；这是可保存、可组合、可发布的非线性编辑器。
...
**一覽：** 可選錄屏與實況標籤 · 7 大類 31 種工具 · 獨立剪輯工程庫 · 多軌時間線 · 裝置端字幕（60+ 語言）· 人臉打碼 · 調色 + LUT · 批次處理 · 6 檔匯出 + 後台佇列 · JKL 專業播放 · Siri · 可選 iCloud 同步。
...
| **剪輯工程** | 獨立工程庫——新建、重新命名、複製，並開啟多軌時間線 |
| **錄屏**（可選，預設關閉） | 開始螢幕錄製；Tab 角標即時顯示已錄時長；可在設定中開啟 |
| **實況照片**（可選，預設開啟） | 實況照片互轉；不用時可在設定中隱藏 |
| **設定** | 主題、語言、匯出、錄屏、儲存、說明、關於、標籤顯隱 |
...
需要真正的時間線剪輯時，開啟 **剪輯工程** 標籤（或從影片庫進入）。工程庫支援重新命名與複製；這是可保存、可組合、可發布的非線性編輯器。
