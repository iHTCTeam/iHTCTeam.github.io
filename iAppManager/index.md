...
iAppManager (爱应用管理) is a **native App Store Connect workstation** for iPhone, iPad and Mac. Sign in with an **API Key**, keep credentials in **Keychain**, and talk **directly to Apple** — versions, screenshots, IAP, TestFlight, certificates, sales reports and customer reviews in one place. **No iHTCTeam account. No ads. No analytics SDK. No third-party backend for your ASC data.**
...
- **Privacy by architecture** — ASC traffic goes to `api.appstoreconnect.apple.com`; iHTCTeam does not host your apps, keys or reports.
...
- **In-App Events**, **App Clips** (default + advanced experiences), **store experiments** (traffic separate from review), **featuring nominations**
- **Availability**, **pricing** (base territory, price points, equalizations preview), **age rating**, **accessibility declarations**, **App Tags**
...
- App Group shares **local** widget snapshots only — never uploaded to iHTCTeam
...
爱应用管理（iAppManager）是面向 iPhone、iPad 与 Mac 的 **原生 App Store Connect 工作台**。用 **API Key** 登录，密钥存 **钥匙串**，请求 **直连 Apple**——版本、截图、内购、TestFlight、证书、销售报表与顾客评价，一处完成。**无需 iHTCTeam 账号。无广告。无分析 SDK。不上架你的 ASC 数据到第三方服务器。**
...
2. 打开爱应用管理 → 填写 Issuer ID / 密钥 ID / PEM（或选择 `.p8`）→ 验证（请求 `GET /v1/apps`）。密钥保存在本机 **钥匙串**。
...
- **架构即隐私** — ASC 流量只到 `api.appstoreconnect.apple.com`；iHTCTeam 不托管你的 App、密钥或报表。
...
- **App 内活动**、**App Clips**（默认 + 高级体验）、**商店实验**（启停流量与提审分开）、**推荐提名**
- **上架地区**、**定价**（基准地区、价格点、equalizations 预览）、**年龄分级**、**辅助功能声明**、**App Tags**
...
- App Group 仅共享 **本地** 小组件摘要——不会上传到 iHTCTeam
- 详见 [隐私政策](privacy)
...
愛應用管理（iAppManager）是面向 iPhone、iPad 與 Mac 的 **原生 App Store Connect 工作台**。用 **API Key** 登入，金鑰存 **鑰匙圈**，請求 **直連 Apple**——版本、截圖、內購、TestFlight、憑證、銷售報表與顧客評價，一處完成。**無需 iHTCTeam 帳號。無廣告。無分析 SDK。不上架你的 ASC 資料到第三方伺服器。**
...
2. 打開愛應用管理 → 填寫 Issuer ID / 金鑰 ID / PEM（或選擇 `.p8`）→ 驗證（請求 `GET /v1/apps`）。金鑰保存在本機 **鑰匙圈**。
...
- **架構即隱私** — ASC 流量只到 `api.appstoreconnect.apple.com`；iHTCTeam 不託管你的 App、金鑰或報表。
...
- **App 內活動**、**App Clips**（預設 + 進階體驗）、**商店實驗**（啟停流量與送審分開）、**推薦提名**
- **上架地區**、**定價**（基準地區、價格點、equalizations 預覽）、**年齡分級**、**輔助功能聲明**、**App Tags**
...
- App Group 僅共享 **本機** 小工具摘要——不會上傳到 iHTCTeam
- 詳見 [隱私權政策](privacy)
