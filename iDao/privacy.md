---
layout: doc
title: iDao — Privacy Policy
app_id: iDao
doc_title_en: Privacy Policy
doc_title_zh_cn: 隐私政策
doc_title_zh_tw: 隱私政策
description: Privacy policy for iDao (爱道德经) — fully offline Tao Te Ching app, Data Not Collected, optional iCloud and Photo Library add-only.
---
...
| Network | Library, reading, recite, Study Hall, Seek the Tao, widgets, and daily notifications work **offline** after install |
| Analytics / ads / tracking | **None** — no third-party analytics, advertising, or tracking SDKs; no App Tracking Transparency use |
| Optional Apple services | **iCloud** (your private KVS, bidirectional merge), **Photo Library (Add Only)** for Dao cards, **StoreKit** for optional tipping, **local notifications**, optional **Live Activities** |
...
- **No content upload** — Your reading history, favorites, lists, notes, mastery, streaks, and preferences stay on your device (and optionally in **your** iCloud). They are never uploaded to iHTC Studio servers because **we do not run such servers for this app**.

## How the App Works

- **Bundled library** — Chapter JSON, vernacular, pinyin, commentaries, English editions, famous lines, and indexes ship inside the app / widget bundle. Browsing and study do not require a network request.
- **On-device speech** — Recitation uses Apple’s system **AVSpeechSynthesizer** (TTS). We do not send chapter text to iHTC Studio.
- **Widgets & App Group** — Widgets and the app share preferences and lightweight state via App Group `group.com.iHTCboy.iDao` on your device.
...
- **Dao cards** — Images are rendered on device. Saving uses **Photo Library add-only** only when you tap save. System Share Sheet may send the image to an app **you** choose; that destination’s privacy policy then applies.
...
| Data | Where | Purpose |
|------|-------|---------|
| Chapter database (incl. pinyin / notes editions) | App / Widget bundle | Offline reading & study |
| Favorites & lists (道单) | App Group UserDefaults | Starred chapters and user lists |
| Reading history / last read | App Group UserDefaults | Continue reading, history UI |
| Study progress & mastery | App Group UserDefaults | Study Hall state, quiz / Seek the Tao progress |
| Notes | App Group UserDefaults | Chapter notes you write |
...
| Daily Chapter cache & notification prefs | App Group UserDefaults | Today’s ID, schedule time, contextual toggle |
...
If you enable **iCloud Sync** in Settings, the app mirrors selected user data through Apple’s **`NSUbiquitousKeyValueStore`** tied to **your Apple ID**, using **bidirectional merge** (favorites, lists, history, mastery, notes, streaks) so cloud and device updates are combined rather than blindly overwritten.
...
iDao requests permissions only when a feature needs them:

| Permission | When | Why |
|------------|------|-----|
| Notifications | You enable Daily Chapter reminders | Local scheduling of reminder notifications |
| Photo Library (Add Only) | You save a Dao card | Write the rendered image; we do not need full library read access |
| iCloud | You enable sync | Sync favorites / lists / history / mastery / notes / streaks |
| Live Activities (iOS) | You enable Reading Live Activity | Show the chapter you are reading on Lock Screen / Dynamic Island |
...
iDao does not knowingly collect personal data from anyone, including children under 13 (or the equivalent age in your region). Because we do not operate a data-collection backend, there is no child profile stored with us. Families may use the reading and study features offline on their own devices.
...
爱火腿肠工作室（iHTC Studio）（「我们」；原 iHTCTeam）开发了 **爱道德经**（iDao / 愛道德經，Bundle ID `com.iHTCboy.iDao`）。本隐私政策说明你在使用本应用时，数据如何被处理（以及**不会**如何被处理）。本页可作为 **App Store 审核**与用户查阅的公开隐私政策。填写 App Store Connect 时，请在浏览器打开本隐私政策页，从地址栏复制当前网址（站点域名日后可能变更，请勿依赖文档中的固定域名）。
...
| 网络 | 语料、阅读、诵读、道学堂、寻道、小组件与每日通知在安装后可**离线**使用 |
| 分析 / 广告 / 追踪 | **无**——无第三方分析、广告或追踪 SDK；不使用跨 App 追踪 |
| 可选的 Apple 服务 | **iCloud**（你的私有键值存储，双向合并）、道卡**相册（仅添加）**、打赏用的 **StoreKit**、**本地通知**、可选**实时活动** |
...
- **无内容上传** — 浏览记录、收藏、道单、笔记、掌握度、打卡与偏好保存在你的设备（以及可选的**你的** iCloud）。它们不会上传到 iHTC Studio 服务器——因为**本应用不为此运营此类服务器**。

## 应用工作原理

- **内置语料** — 章节、白话、拼音、注解、英译、名句与索引随 App / Widget 安装包提供，浏览与学堂无需联网。
- **本机语音合成** — 诵读使用系统 **AVSpeechSynthesizer**，我们不会把正文发送到 iHTC Studio。
- **小组件与 App Group** — App 与小组件通过本机 App Group `group.com.iHTCboy.iDao` 共享偏好与轻量状态。
- **每日通知** — 使用本地用户通知调度；点击后通过深链打开本机章节，无服务器推送内容库。
- **道卡分享** — 图片在设备上渲染。仅当你点「保存」时请求**相册仅添加**权限。系统分享会把图片交给**你选择**的目标 App，此后适用该目标的隐私政策。
...
| 数据 | 位置 | 用途 |
|------|------|------|
| 章节语料（含拼音 / 注解版本） | App / Widget Bundle | 离线阅读与学堂 |
| 收藏与道单 | App Group UserDefaults | 星标与自定义道单 |
| 浏览记录 / 最近阅读 | App Group UserDefaults | 继续阅读与历史 |
| 学堂进度与掌握度 | App Group UserDefaults | 道学堂状态、测验 / 寻道进度 |
| 笔记 | App Group UserDefaults | 你写下的章节笔记 |
| 打卡 / 连续天数 | App Group UserDefaults | 每日习惯 |
| 外观与阅读偏好 | App Group UserDefaults | 主题、强调色、语言、布局、TTS、Tab、小组件 |
| 每日一章缓存与通知偏好 | App Group UserDefaults | 今日 ID、提醒时间、情境开关 |
| 道卡偏好 | App Group UserDefaults | 上次风格 / 比例 |
...
若在设置中开启 **iCloud 同步**，应用通过绑定**你的 Apple ID** 的 **`NSUbiquitousKeyValueStore`** 镜像下列用户数据，并做**双向合并**（收藏、道单、浏览记录、掌握度、笔记、打卡），避免粗暴覆盖：
...
|------|------|------|
| 通知 | 你开启每日一章提醒 | 本地调度提醒 |
| 相册（仅添加） | 你保存道卡 | 写入渲染图片；无需读取全部相册 |
| iCloud | 你开启同步 | 同步收藏 / 道单 / 历史 / 掌握度 / 笔记 / 打卡 |
| 实时活动（iOS） | 你开启品读实时活动 | 在锁屏 / 灵动岛显示正在品读的章节 |
...
- **不**在你主动保存道卡之外访问相册（且仅为「添加」）。
- **不**接收 StoreKit 银行卡号（由 Apple 处理支付）。

## 儿童隐私

爱道德经不会故意收集包括 13 岁以下（或你所在地区同等年龄）儿童在内的任何人的个人数据。由于我们不运营数据收集后端，我们这边不会保存儿童档案。家庭可在自有设备上离线使用阅读与学堂功能。
...
愛火腿腸工作室（iHTC Studio）（「我們」；原 iHTCTeam）開發了 **愛道德經**（iDao / 爱道德经，Bundle ID `com.iHTCboy.iDao`）。本隱私政策說明你在使用本應用時，資料如何被處理（以及**不會**如何被處理）。本頁可作為 **App Store 審核**與使用者查閱的公開隱私政策。填寫 App Store Connect 時，請在瀏覽器開啟本隱私政策頁，從網址列複製目前網址（網站網域日後可能變更，請勿依賴文件中的固定網域）。
...
| 網路 | 語料、閱讀、誦讀、道學堂、尋道、小工具與每日通知在安裝後可**離線**使用 |
| 分析 / 廣告 / 追蹤 | **無**——無第三方分析、廣告或追蹤 SDK；不使用跨 App 追蹤 |
| 可選的 Apple 服務 | **iCloud**（你的私有鍵值儲存，雙向合併）、道卡**照片（僅加入）**、打賞用的 **StoreKit**、**本地通知**、可選**即時動態** |
...
- **無內容上傳** — 瀏覽記錄、收藏、道單、筆記、掌握度、打卡與偏好保存在你的裝置（以及可選的**你的** iCloud）。它們不會上傳到 iHTC Studio 伺服器——因為**本應用不為此營運此類伺服器**。

## 應用工作原理

- **內建語料** — 章節、白話、拼音、註解、英譯、名句與索引隨 App / Widget 安裝包提供，瀏覽與學堂無需連網。
- **本機語音合成** — 誦讀使用系統 **AVSpeechSynthesizer**，我們不會把正文傳送到 iHTC Studio。
- **小工具與 App Group** — App 與小工具透過本機 App Group `group.com.iHTCboy.iDao` 共享偏好與輕量狀態。
- **每日通知** — 使用本地使用者通知排程；點擊後透過深鏈開啟本機章節，無伺服器推播內容庫。
- **道卡分享** — 圖片在裝置上渲染。僅當你點「儲存」時請求**照片僅加入**權限。系統分享會把圖片交給**你選擇**的目標 App，此後適用該目標的隱私政策。
...
| 資料 | 位置 | 用途 |
|------|------|------|
| 章節語料（含拼音 / 註解版本） | App / Widget Bundle | 離線閱讀與學堂 |
| 收藏與道單 | App Group UserDefaults | 星號與自訂道單 |
| 瀏覽記錄 / 最近閱讀 | App Group UserDefaults | 繼續閱讀與歷史 |
| 學堂進度與掌握度 | App Group UserDefaults | 道學堂狀態、測驗 / 尋道進度 |
| 筆記 | App Group UserDefaults | 你寫下的章節筆記 |
| 打卡 / 連續天數 | App Group UserDefaults | 每日習慣 |
| 外觀與閱讀偏好 | App Group UserDefaults | 主題、強調色、語言、版面、TTS、Tab、小工具 |
| 每日一章快取與通知偏好 | App Group UserDefaults | 今日 ID、提醒時間、情境開關 |
| 道卡偏好 | App Group UserDefaults | 上次風格 / 比例 |
...
若在設定中開啟 **iCloud 同步**，應用透過綁定**你的 Apple ID** 的 **`NSUbiquitousKeyValueStore`** 鏡像下列使用者資料，並做**雙向合併**（收藏、道單、瀏覽記錄、掌握度、筆記、打卡），避免粗暴覆蓋。
...
|------|------|------|
| 通知 | 你開啟每日一章提醒 | 本地排程提醒 |
| 照片（僅加入） | 你儲存道卡 | 寫入渲染圖片；無需讀取全部照片 |
| iCloud | 你開啟同步 | 同步收藏 / 道單 / 歷史 / 掌握度 / 筆記 / 打卡 |
| 即時動態（iOS） | 你開啟品讀即時動態 | 在鎖定畫面 / 動態島顯示正在品讀的章節 |
...
- **不**在你主動儲存道卡之外存取照片（且僅為「加入」）。
- **不**接收 StoreKit 银行卡／信用卡號碼（由 Apple 處理付款）。

## 兒童隱私

愛道德經不會故意收集包括 13 歲以下（或你所在地區同等年齡）兒童在內的任何人的個人資料。由於我們不營運資料收集後端，我們這邊不會保存兒童檔案。家庭可在自有裝置上離線使用閱讀與學堂功能。
